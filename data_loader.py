...
@app.cell(hide_code=True)
def _():
    patch_size_LR_input = mo.ui.text(value="16", label="LR Patch Size (mm):")
    patch_size_PA_input = mo.ui.text(value="16", label="PA Patch Size (mm):")
    patch_size_IS_input = mo.ui.text(value="1", label="IS Patch Size (mm):")
    n_patches_input = mo.ui.text(value="64", label="Num Patches:")

    run_button = mo.ui.run_button()
    mo.vstack([patch_size_LR_input, patch_size_PA_input, patch_size_IS_input, n_patches_input, run_button])
    return (
        n_patches_input,
        patch_size_IS_input,
        patch_size_LR_input,
...
def _(
    ...
    if run_button.value:
        patch_size_LR = int(patch_size_LR_input.value)
        patch_size_PA = int(patch_size_PA_input.value)
        patch_size_IS = int(patch_size_IS_input.value)
        ...
        patch_shape = (patch_size_LR, patch_size_PA, patch_size_IS)
        scan = nifti_scan(path_to_scan=nifti_name, median=metadata_row["median"].values[0], stdev=metadata_row["stdev"].values[0])
    return metadata_row, n_patches, patch_shape, scan


@app.cell
def _(n_patches, patch_shape, scan):
...
def _(final_patches, scan_max, scan_min, sld):
    patch_display = final_patches[sld.value, :, :, 0].copy()
...
def _(center_point_vox, final_patches, patch_centers_vox, scan, sld):
    if sld.value == final_patches.shape[0]:
        slider_value = value=center_point_vox[2]
    else:
        slider_value = value=patch_centers_vox[sld.value][2]
    slider = mo.ui.slider(start=0, stop=scan.nii_data.shape[2]-1, value=slider_value)
    slider
    return (slider,)


@app.cell
def _(scan_data_copy, slider):
    mo.image(src=scan_data_copy[:,:,slider.value]) #slider.value])
    return
...
class nifti_scan():
    ...
    def __init__(self, path_to_scan, median, stdev):
    ...
    def train_sample(
        ...
        *, # Force subsequent arguments to be keyword-only for clarity
        patch_shape: Optional[Tuple[int, int, int]] = (16, 16, 1),
        ...
        if subset_center is None:
            subset_center = self.get_random_center_idx(sampling_radius_mm, patch_shape)
